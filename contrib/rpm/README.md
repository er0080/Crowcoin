...
The RPM spec file provided here is for Crow-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://crow.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Crow git repository](https://github.com/CrowProject/Crowcoin) but are not part of
...
Currently only `Source10` is used, for the example `crow.conf` file.
...
in the crow git repository. At present only an SVG version of the Crow
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the crow
...
needed when 0.12.1 is released, a proper fix is already in the Crow git
master branch.
...
idea to build Crow Core against a static version of that library compiled
...
The `crow.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `crow-server` package. At this point, most RPM based
...
The `crow-server` package should have SELinux support. How to properly do
...
Crow-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/crow/). He has been
packaging Crow for Fedora at least since 2012.
...
4. I renamed the `crow` package that contains the Qt GUI to `crow-core` as
that appears to be how the general population refers to it, in contrast to
`crow-xt` or `crow-classic`. I wanted to make sure the general population
