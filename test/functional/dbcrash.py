...
class ChainstateWriteCrashTest(CrowTestFramework):
    ...
    def restart_node(self, node_index, expected_tip):
        ...
        while time.time() - time_start < 120:
            try:
                ...
                # If crowd exits, then try again.  wait_for_node_exit()
                # should raise an exception if crowd doesn't exit.
                self.wait_for_node_exit(node_index, timeout=10)
            self.crashed_on_restart += 1
            time.sleep(1)

        # If we got here, crowd isn't coming back up on restart.  Could be a
        # bug in crowd, or we've gotten unlucky with our dbcrash ratio --
