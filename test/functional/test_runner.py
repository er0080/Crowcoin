...
`test/functional/test_framework/test_framework.py:CrowTestFramework.main`.
...
def main():
    ...
    tmpdir = "%s/crow_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    if not (enable_wallet and enable_utils and enable_crowd):
        print("No functional tests to run. Wallet, utils, and crowd must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, jobs=1, enable_coverage=False, args=[]):
    # Warn if crowd is already running (unix only)
    try:
        if subprocess.check_output(["pidof", "crowd"]) is not None:
            print("%sWARNING!%s There is already a crowd process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    #Set env vars
    if "CROWD" not in os.environ:
        os.environ["CROWD"] = build_dir + '/src/crowd' + exeext
        os.environ["CROWCLI"] = build_dir + '/src/crow-cli' + exeext
...
class TestHandler:
    ...
    def __init__(self, num_tests_parallel, tests_dir, tmpdir, test_list=None, flags=None):
        ...
        # In case there is a graveyard of zombie crowds, we can apply a
...
class RPCCoverage():
    ...
    commands per `crow-cli help` (`rpc_interface.txt`).
